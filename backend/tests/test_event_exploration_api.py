...
from app.db.models import Actor, Document, Event, EventActor, EventSource, EventType, Location
...
def _seed_events(client: TestClient) -> dict[str, str]:
    with client.app.state.session_factory() as session:
        ...
        matching.event_sources.append(EventSource(source=document, evidence_quote="Source text."))
        other = Event(
            title="City protest",
        ...
        session.add_all([matching, other, unpublished])
        session.commit()
        return {"event": matching.id, "type": airstrike.id, "actor": air_force.id, "document": document.id}


def test_published_event_query_combines_search_relation_and_date_filters(tmp_path: Path) -> None:
...
def test_review_status_approved_is_accepted_as_a_deprecated_alias_for_published(
    tmp_path: Path,
) -> None:
    client = _client(tmp_path)
    ids = _seed_events(client)
    ...
    response = client.get(
        "/api/events",
        params={"review_status": "approved", "event_type_id": ids["type"], "country": "YE"},
    )
...
def test_dashboard_summary_uses_the_same_filtered_events(tmp_path: Path) -> None:
    client = _client(tmp_path)
    ids = _seed_events(client)
