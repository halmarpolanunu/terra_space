...
**Two live tests remain** before this plan's completion gate is satisfied: a deliberately eventless article (expect `COMPLETED_NO_CANDIDATE`, zero Phase 3 rows) and a blank `p1_raw_content_text` on Phase 1 directly (expect failure before any Supabase write). See the n8n transition plan's "Owner handoff" section for exact steps.

**Next action:** the owner runs the two remaining tests above, then this plan can move to `completed` and plan 3 (Terra Space application transition) can begin.
