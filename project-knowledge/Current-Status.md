...
**Next action:** ask the owner to (1) re-run `Terra Space - Event Records` for `p1_uuid 81322f2f-a959-438e-a77e-0ce5adc0f582` again unchanged, to prove idempotency, and (2) decide how to run the human-edit-survives-rerun check — the Dashboard doesn't exist yet (that's plan 3), so the only way to simulate a human edit right now is a direct, explicitly-confirmed SQL `UPDATE` to one `phase3_events` row.
