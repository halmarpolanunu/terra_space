...
describe("Navigation", () => {
  it("offers four clear destinations and current supporting views", () => {
    ...
    for (const href of ["/event-review", "/sense/event-types", "/sense/actors"]) {
      expect(screen.getAllByRole("link").some((link) => link.getAttribute("href") === href)).toBe(true);
    }
    for (const href of ["/issues", "/events", "/dashboard"]) {
