    ...
    {state.kind === "ready" && <>
      {stories.length === 0 ? <p className={styles.notice}>No current Phase 2 Main Issues yet.</p> : selectedIssue ? <>
        <div className={styles.issueStage}>
          <section id="explore-place" className={styles.map} aria-label="Full Issue event map"><div className={styles.mapHeader}><div className={styles.heading}><span>01 / ISSUE PLACES</span><h2>Related places.</h2><p className={styles.mapIssueLabel}>{selectedIssue.label}</p></div><div className={styles.mapTelemetry}><div><strong>{issuePlaces.length}</strong><span>unique places</span></div><div><strong>{scopedSummary.total - scopedSummary.mapped}</strong><span>events unmapped</span></div></div></div>
            ...
            <p className={styles.caption}>All linked event locations are included, even outside {selectedCountry?.name ?? "the starting country"}. Only resolved Event Geography appears on the map; actor locations are excluded.</p>
          </section>
