...
import { listPipelineReviews, listBridgeSources, listPhase5Events, type PipelineReview, type BridgeSource, type Phase5Event } from "@/lib/bridge-api";
...
export function PrepareWorkspace() {
  ...
  const [reviews, setReviews] = useState<Load<PipelineReview[]>>({ kind: "loading" });
  ...
  const load = useCallback(() => {
    ...
    void listPipelineReviews().then((data) => setReviews({ kind: "ready", data })).catch(() => setReviews({ kind: "error" }));
  ...
  return <AppShell currentPath="/prepare"><div className={styles.prepare}>
    ...
    <section className={styles.attentionPanel}><h2>Attention and next steps</h2>
      ...
      {events.kind === "ready" && events.data.filter((event) => event.phase4_status === "NEEDS_REVIEW" || event.phase4_status === "FAILED" || event.classification.status !== "CLASSIFIED" || event.qualification.status !== "FINAL").slice(0, 8).map((event) => <p key={event.id}><strong>{event.title}</strong>: {event.phase4_status === "NEEDS_REVIEW" || event.phase4_status === "FAILED" ? "event facts need review" : event.classification.status !== "CLASSIFIED" ? "event type unclassified" : event.qualification.status === "NOT_FINAL" ? "not final" : "qualification pending"}. <Link href={`/explore?event=${encodeURIComponent(event.id)}`}>Inspect event</Link></p>)}
    ...
    {reviews.kind === "ready" && <section className={styles.attentionPanel} id="candidates"><h2>Retained candidate reviews</h2>
      {reviews.data.length === 0 ? <p>No candidate reviews retained yet.</p> : reviews.data.map((review) => <div key={review.phase1_source_id}>
        ...
        {review.event_candidates.length > 0 && <ul>{review.event_candidates.map((candidate, index) => <li key={`${review.phase1_source_id}-${index}`}>{candidate.title} · {candidate.status.replaceAll("_", " ")}</li>)}</ul>}
      </div>)}
    </section>}
  </div></AppShell>;
}
