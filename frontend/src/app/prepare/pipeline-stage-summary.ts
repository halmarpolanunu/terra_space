...
export function summarizePipelineStages(sources: BridgeSource[], reviews: PipelineReview[], events: Phase5Event[]): PipelineStageSummary[] {
  ...
  return [
    ...
    { id: "issues", label: "Main Issue", total: uniqueReviews.length, attention: uniqueReviews.filter((review) => review.main_issue_status === "FAILED" || review.main_issue_status === "NEEDS_REVIEW").length, attentionScope: "source reviews", scope: "reviewed sources", href: "/explore", description: "The central issue found in each processed source." },
