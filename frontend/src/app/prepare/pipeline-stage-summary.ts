...
export type PipelineStageSummary = {
  ...
  label: string; total: number; attention: number; attentionScope: string; scope: string; href: string; description: string;
};
...
export function summarizePipelineStages(sources: BridgeSource[], reviews: BridgeCandidateReview[], events: Phase5Event[]): PipelineStageSummary[] {
  ...
  return [
    ...
    { id: "issues", label: "Main Issue", total: uniqueReviews.length, attention: uniqueReviews.filter((review) => review.main_issue_status === "FAILED").length, attentionScope: "source reviews", scope: "reviewed sources", href: "/issues", description: "The central issue found in each processed source." },
    { id: "candidates", label: "Event Candidates", total: candidates.length, attention: uniqueReviews.filter((review) => review.event_detection_status === "FAILED" || review.event_detection_status === "NOT_RUN").length, attentionScope: "source reviews", scope: "candidate records", href: "/prepare#candidates", description: "Possible events retained from source text." },
    { id: "facts", label: "Event Facts", total: uniqueEvents.length, attention: uniqueEvents.filter((event) => event.phase4_status !== "PREPARED").length, attentionScope: "Phase 5 records", scope: "Phase 5 event records", href: "/explore", description: "Grounded event details, dates, and geography." },
