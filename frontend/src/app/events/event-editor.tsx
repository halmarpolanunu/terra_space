  ...
  return (
    <FramedPanel title="Edit event">
      <form onSubmit={submit}>
        ...
        <div className="field"><label htmlFor="event-epistemic-status">Epistemic status</label><select id="event-epistemic-status" onChange={(e) => setEpistemicStatus(e.target.value as EventRead["epistemic_status"])} value={epistemicStatus}>{["confirmed", "reported", "alleged", "planned", "denied", "unknown"].map((status) => <option key={status} value={status}>{status}</option>)}</select></div>
