-- Expose grounded history for conservative recovery from technical model failures.
...
select
  ...
  baseline_date.facts as baseline_phase4_date_facts,
  stable.facts as stable_phase4_facts
...
left join lateral (
  select run.facts
  from public.terra_space_phase4_event_fact_processing_runs as run
  where run.phase3_event_candidate_result_id = phase3.id
    and run.candidate_id = candidate.value ->> 'candidate_id'
    and run.facts ->> 'event_date' is not null
  order by run.run_id desc
  limit 1
) as baseline_date on true
left join lateral (
  select run.facts
  from public.terra_space_phase4_event_fact_processing_runs as run
  where run.phase3_event_candidate_result_id = phase3.id
    and run.candidate_id = candidate.value ->> 'candidate_id'
    and run.extraction_prompt_version = 'phase4-narrow-extraction-v2'
    and run.extraction_raw_output is not null
    and run.extraction_raw_output not like '%"date_epistemic":null%'
    and run.extraction_raw_output not like '%"actors":null%'
    and run.extraction_raw_output not like '%"locations":null%'
  order by run.run_id desc
  limit 1
) as stable on true
...
comment on view public.terra_space_phase4_narrow_pilot_candidates is
  'Temporary five-candidate Phase 4 input with grounded history for conservative technical-failure recovery.';
