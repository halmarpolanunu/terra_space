-- Expose the last Phase 4 v1 facts as a stable date baseline for the temporary pilot.
...
select
  ...
  baseline.facts as baseline_phase4_date_facts
...
left join lateral (
  ...
  where run.phase3_event_candidate_result_id = phase3.id
    ...
    and run.extraction_prompt_version = 'phase4-event-fact-extraction-v1'
...
comment on view public.terra_space_phase4_narrow_pilot_candidates is
  'Temporary five-candidate Phase 4 input including the stable v1 date baseline for conservative rerun comparison.';
